...
import frc.robot.auto.DriveMotionPlanner;
...
import frc.robot.lib.team254.trajectory.timing.TimedState;
import frc.robot.lib.team254.trajectory.timing.TimingConstraint;
...
public class TrajectoryGenerator {
  ...
  public class TrajectorySet {
    ...
    private Trajectory<TimedState<Pose2dWithCurvature>> getHumanLoaderToMidShip(){
      List<Pose2d> waypoints = new ArrayList<>();
      waypoints.add(humanLoaderPose);
      waypoints.add(new Pose2d(portScoringPose.transformBy(Pose2d.fromTranslation(
          new Translation2d(-6.0, 0.0))).getTranslation(), Rotation2d.fromDegrees(0.0)));
      waypoints.add(midShipScoringPose);

      return generateTrajectory(false, waypoints, Arrays.asList(), 
      120.0, kMaxAccel, 24.0, kMaxVoltage, 72.0, 60);
    }
  }
